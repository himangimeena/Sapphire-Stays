    ...
    <div className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 animate-fade-in space-y-10 text-slate-900 dark:text-slate-100">
      <div className="border-b border-gray-200 dark:border-gray-800 pb-6 flex justify-between items-center">
        <div>
          ...
          <h1 className="font-serif text-4xl font-bold mt-1 text-slate-900 dark:text-slate-100">Sapphire Palace Udaipur – Manager Portal</h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card p-6 rounded-2xl border border-gray-200 dark:border-gray-800 space-y-1 text-slate-900 dark:text-slate-100">
          <span className="text-xs text-slate-600 dark:text-slate-400 uppercase font-bold">Total Suites</span>
          <span className="font-serif text-3xl font-bold text-slate-900 dark:text-slate-100">42</span>
        </div>
        <div className="glass-card p-6 rounded-2xl border border-gray-200 dark:border-gray-800 space-y-1 text-slate-900 dark:text-slate-100">
          <span className="text-xs text-slate-600 dark:text-slate-400 uppercase font-bold">Occupied Today</span>
          <span className="font-serif text-3xl font-bold text-[#0F3D6E] dark:text-amber-300">36</span>
        </div>
        <div className="glass-card p-6 rounded-2xl border border-gray-200 dark:border-gray-800 space-y-1 text-slate-900 dark:text-slate-100">
          <span className="text-xs text-slate-600 dark:text-slate-400 uppercase font-bold">Average Nightly Rate</span>
          <span className="font-serif text-3xl font-bold text-slate-900 dark:text-slate-100">₹58,500</span>
        </div>
      </div>
      ...
      <div className="glass-card p-6 rounded-2xl border border-gray-200 dark:border-gray-800 space-y-6 text-slate-900 dark:text-slate-100">
        <h3 className="font-serif text-2xl font-bold text-slate-900 dark:text-slate-100">Room Inventory & Status Board</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {rooms.map(r => (
            <div key={r.id} className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 space-y-3 text-slate-900 dark:text-slate-100">
              <div className="flex justify-between items-center">
                <span className="font-serif font-bold text-lg text-slate-900 dark:text-slate-100">Room {r.room_number}</span>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">{r.type_name}</p>
              <div className="pt-2 border-t flex justify-between text-xs font-semibold text-slate-700 dark:text-slate-300">
                <span>Rate</span>
