...
router.get('/housekeeping', authenticate, requireRoles(['SUPER_ADMIN', 'BRANCH_ADMIN', 'RECEPTIONIST']), async (req, res) => {
  try {
...
router.patch('/housekeeping/:id', authenticate, requireRoles(['SUPER_ADMIN', 'BRANCH_ADMIN', 'RECEPTIONIST']), async (req, res) => {
  try {
...
router.get('/maintenance', authenticate, requireRoles(['SUPER_ADMIN', 'BRANCH_ADMIN', 'RECEPTIONIST']), async (req, res) => {
  try {
...
router.post('/maintenance', authenticate, requireRoles(['SUPER_ADMIN', 'BRANCH_ADMIN', 'RECEPTIONIST']), async (req, res) => {
  try {
...
router.patch('/maintenance/:id', authenticate, requireRoles(['SUPER_ADMIN', 'BRANCH_ADMIN', 'RECEPTIONIST']), async (req, res) => {
  try {
    const { status, roomId } = req.body;
